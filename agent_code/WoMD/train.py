...
def grad_q_hat(self,S,A,w):
    ...
    for i in range(len(w)):#use that q is just linear w.r.t. w
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state == None: #avoid bug
        ...
        #trigger event when agent does a step towards a safe place
        if old_features[9]<new_features[9] and old_features[9]!=0:
            events.append(TOWARDS_SAFE_PLACE)

        #trigger event for getting closer to a crate
        for i in range(3):
        ...
        ###first crucial distinction: which algorithm to choose.
        ...
        #check how the weights correction works
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    #trigger event if agents just kills himself right at the beginning
    if S['step']<=10 and 'KILLED_SELF' in events:
        events.append(EARLY_SUICIDE)
    ...
    #sum the rewards that are left of n-step algorithms
    Reward=0
    for transition in self.transitions:
        Reward+=gamma*transition[3]
